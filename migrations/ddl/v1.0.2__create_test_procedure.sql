...
CREATE OR REPLACE PROCEDURE <env>_db.<env>_schema.sp_create_cicd_test()
RETURNS STRING
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
  ...
  CREATE TABLE IF NOT EXISTS <env>_db.<env>_schema.cicd_test_table (
  ...
  INSERT INTO <env>_db.<env>_schema.cicd_test_table (name) VALUES ('ci-test');
...
CALL <env>_db.<env>_schema.sp_create_cicd_test();
